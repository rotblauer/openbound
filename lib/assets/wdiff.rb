...
# https://github.com/zemis/wdiff/blob/master/lib/wdiff.rb
...
module Wdiff
  class << self
    ...
    def diff(old_str, new_str, options={})
      ...
      # execute command on tempfiles
      # 
      %x{#{bin_path} #{opt_str} #{f1.path} #{f2.path}} # add -n flag to avoid newlines and handle each line individually
    ensure
      f1.close
      f1.unlink
      f2.close
  ...
  module Helper
    def self.to_html(old_str,new_str)
      ...
      Wdiff::diff(old_str, new_str, :inserts => ["<span class='diffins'>", "</span>"], :deletes => ["<span class='diffout'>", "</span>"])
    end
  end
end
