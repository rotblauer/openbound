...
#       :bla => Blah
    ...
    tables.each do |tbl|
      ...
      conn.run_sql "ALTER TABLE #{tbl} #{a.join(', ')}" if a.present?
  ...
  desc "convert a utf8 database with utf8 data into proper utf8mb4"
  task :convert_to_utf8 => :environment do
    puts Time.now
    dryrun = ENV['DOIT'] != '1'
    conn = ActiveRecord::Base.connection
    ...
    tables.each do |tbl|
      ...
      unless tables_without_models.include? tbl
        ...
        model.columns.each do |col|
          ...
          when /varchar/
            a << "CHANGE #{conn.quote_column_name(col.name)} #{conn.quote_column_name(col.name)} VARBINARY(#{col.limit})"
            b << "CHANGE #{conn.quote_column_name(col.name)} #{conn.quote_column_name(col.name)} VARCHAR(#{col.limit}) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci#{nullable}#{default}"
          when /text/
            a << "CHANGE #{conn.quote_column_name(col.name)} #{conn.quote_column_name(col.name)} BLOB"
            b << "CHANGE #{conn.quote_column_name(col.name)} #{conn.quote_column_name(col.name)} TEXT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci#{nullable}#{default}"
          when /varbinary/
            b << "CHANGE #{conn.quote_column_name(col.name)} #{conn.quote_column_name(col.name)} VARCHAR(#{col.limit}) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci#{nullable}#{default}"
          when /blob/
            b << "CHANGE #{conn.quote_column_name(col.name)} #{conn.quote_column_name(col.name)} TEXT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci#{nullable}#{default}"
